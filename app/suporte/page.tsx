...
export default function SuportePage(){
 ...
 const socialReply=(input:string)=>{const q=input.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim().replace(/[!?.,]+$/g,'');if(/^(oi|ola|oie|opa|hey|bom dia|boa tarde|boa noite)$/.test(q))return 'Oi! Tudo bem? Como posso te ajudar hoje?';if(/^(tudo bem|td bem|como vai|como voce esta|ta tudo bem)$/.test(q))return 'Tudo bem por aqui 😊 E com você? Me conta, como posso te ajudar hoje?';if(/^(obrigado|obrigada|valeu|agradeco)$/.test(q))return 'Por nada! Estou por aqui sempre que precisar.';return null};
 ...
 const sendChat=async(event?:FormEvent)=>{event?.preventDefault();const text=chatText.trim();if(!text||chatThinking)return;const next=[...chat,{from:'user',text}] as {from:string;text:string}[];setChat(next);setChatText('');const social=socialReply(text);if(social){setChat(items=>[...items,{from:'leo',text:social}]);return}setChatThinking(true);try{const response=await fetch('/api/suporte/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:next})});const data=await response.json();if(!response.ok||!data.reply)throw new Error(data.error||'support error');setChat(items=>[...items,{from:'leo',text:data.reply}])}catch{setChat(items=>[...items,{from:'leo',text:supportReply(text)}])}finally{setChatThinking(false)}};
